 ...
 * @Description:
 */
public class QueryRealtimeFlatMapper3h extends RichFlatMapFunction<ItemFeatureEntity, String> {

    /**
	 * 
	 */
	private static final long serialVersionUID = -8284916412921643582L;
	private static Logger log = LoggerFactory.getLogger(QueryRealtimeFlatMapper3h.class);
    ...
    public void flatMap(ItemFeatureEntity value, Collector<String> collector) {
		...
		try {
	        ...
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h|"+JSONObject.toJSONString(queryClickMap));
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3ht|"+windowEnd);
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_rf|"+JSONObject.toJSONString(queryClickRfMap));
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_rank|"+JSONObject.toJSONString(queryClickRankMap));
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_rank_rf|"+JSONObject.toJSONString(queryClickRankRfMap));
	        collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_ratio|"+JSONObject.toJSONString(queryClickratioMap));

	        if (queryCtrMap.size() != 0) {
	        	collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_ctr|"+JSONObject.toJSONString(queryCtrMap));
	        	queryCtrRankMap = Utils.mapOrder(queryCtrMap);
	        	collector.collect(windowEnd+"|rqac_" + queryIdkey+"|"+queryId+"_3h_ctr_rank|"+JSONObject.toJSONString(queryCtrRankMap));
	        	}
        	} catch (Exception e) {

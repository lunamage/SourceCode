...
public class MallFeatureRedisSink8h extends RichSinkFunction<MallFeature.ItemFeatureEntity> {
	
	private static Logger log = LoggerFactory.getLogger(MallFeatureRedisSink8h.class);
    ...
    public void open(Configuration parameters) {
	    ...
	    public void invoke(MallFeature.ItemFeatureEntity value, SinkFunction.Context context) {
	        ...
	        if(impMap.size()!=0){
	        	RedisMap.put("8h_imp", JSONObject.toJSONString(impMap));
		        RedisMap.put("8h_impht", windowEnd);
	        }
	        
	        if(secMap.size()!=0){
	        	RedisMap.put("8h_sec", JSONObject.toJSONString(secMap));
		        RedisMap.put("8h_secht", windowEnd);
	        }
